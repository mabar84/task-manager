    ...
    switch (action.type) {
        ...
        case 'ADD-TASK': {
            return state.map(tl => tl.id === action.taskListId
                ? {
                    ...tl,
                    tasks: [{id: tl.tasks.length + 1, text: action.title, isFinished: false},
                        ...tl.tasks]
                }
                : tl)
        }
...
export const addTaskAC = (taskListId: number, title: string) => ({type: 'ADD-TASK' as const, taskListId, title})
